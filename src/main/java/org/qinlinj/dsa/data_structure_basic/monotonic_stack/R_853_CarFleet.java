...
// [853] Car Fleet
public class R_853_CarFleet {
    public static void main(String[] args) {
        Solution solution = new R_853_CarFleet().new Solution();
...
class Solution {
    public int carFleet(int target, int[] position, int[] speed) {
        int n = speed.length;
        int[][] carInfo = new int[n][2];
        for (int i = 0; i < n; i++) {
            carInfo[i] = new int[]{position[i], speed[i]};
        }
        Arrays.sort(carInfo, (a, b) -> a[0] - b[0]);

        ArrayDeque<Double> stk = new ArrayDeque<>();
        for (int i = n - 1; i >= 0; i--) {
            double currentTime = (double) (target - carInfo[i][0]) / carInfo[i][1];
...
//  
// 
//  Related Topics Array Stack Sorting Monotonic Stack 👍 4274 👎 1210
