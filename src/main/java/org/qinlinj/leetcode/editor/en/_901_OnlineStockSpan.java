...
public class _901_OnlineStockSpan {
    public static void main(String[] args) {
        StockSpanner solution = new _901_OnlineStockSpan().new StockSpanner();
...
class StockSpanner {
    ...
    public int next(int price) {
        // find the first price[i] greater than price[j] on the left
        while (!stk.isEmpty() && stk.peek()[0] <= price) {
            stk.pop();
        }
        int span = stk.isEmpty() ? index + 1 : index - stk.peek()[1];
...
//  
// 
//  Related Topics Stack Design Monotonic Stack Data Stream 👍 7218 👎 500
